...
use crate::types::{
    _assoc, error, func, hash_map, list, unwrap_map_key, vector, wrap_map_key, FuncStruct, MalArgs,
    MalRet, MalVal,
};
use readline;
...
macro_rules! fn_fold_int {
    ($init:expr, $fn:expr) => {{
        |a: MalArgs| {
            let mut result = $init;
            for arg in a.iter() {
                match arg {
                    Int(val) => result = $fn(result, *val),
                    _ => return error("expecting int args"),
                }
            }
            Ok(Int(result))
        }
...
fn divisione_variadica(a: MalArgs) -> MalRet {
    if a.is_empty() {
        return error("(/) richiede almeno un argomento");
    }

    let mut corrente = match a[0] {
        Int(val) => val,
        _ => return error("L'argomento deve essere un intero"),
    };
...
pub fn ns() -> Vec<(&'static str, MalVal)> {
    vec![
        ...
        ("+", func(fn_fold_int!(0, |i, j| i + j))),
        ("*", func(fn_fold_int!(1, |i, j| i * j))),
        ("/", func(divisione_variadica)),
